...
bool FPC1020A::begin(HardwareSerial *serial, uint8_t rx, uint8_t tx,
                        unsigned long baud) {
    _serial = serial;
...
bool FPC1020A::enableDebug(HardwareSerial *debug_serial) {
    _debug        = debug_serial;
    _enable_debug = true;
    return true;
}

bool FPC1020A::setBaud(unsigned long baud) {
    uint8_t res;
    TxBuf[CMD] = CMD_BAUD;
    TxBuf[P1]  = 0;
    TxBuf[P2]  = 0;
...
/*! @brief Send a message and wait for the specified time until the return value
 is received.*/
uint8_t FPC1020A::sendCMD(uint16_t timeout) {
    uint8_t i, j;
    uint8_t checkSum = 0;
    memset(RxBuf, 0, 9);
    TxBuf[5] = 0;
...
bool FPC1020A::setFingerMode(fpc1020a_add_finger_mode_t mode) {
    uint8_t res;
...
uint8_t FPC1020A::getFingerMode(void) {
    TxBuf[CMD] = CMD_ADD_MODE;
    TxBuf[P1]  = 0;
    TxBuf[P2]  = 0;
    TxBuf[P3]  = 0x01;
...
uint16_t FPC1020A::getUserCount(void) {
    uint8_t res;
...
bool FPC1020A::delFinger(uint8_t id) {
    uint8_t res;
...
/*! @brief Add User.*/
bool FPC1020A::addFinger(uint8_t id, uint8_t permission,
...
uint8_t FPC1020A::getFingerID(void) {
